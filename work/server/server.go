...
func (sg *Server) Serve(request *http.Request, writer http.ResponseWriter) {
	...
	if e := request.FormValue(EchoStr); e != "" {
		...
		// 验证url
		if !encrypt.ValidMsgSignature(timestamp, nonce, e, msgSignature) {
			log.Println("msg_signature is invalid")
			return
		}

		// Decode base64
		cipherData, err := base64.StdEncoding.DecodeString(e)
		if err != nil {
		...
		log.Printf("%s Decrypt: %s", EchoStr, string(plainData))
